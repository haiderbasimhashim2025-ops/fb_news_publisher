...
# -*- coding: utf-8 -*-
"""
نظام نشر الأخبار التلقائي على Facebook - النسخة 17
...
def save_to_db(news):
    ...
    try:
        cursor.execute('''
            INSERT INTO news (title, description, link, image_url, saved_at, posted_at)
            VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP, NULL)
        ''', (news['title'], news['description'], news['link'], news.get('image_url')))
...
def main_loop():
    """الحلقة الرئيسية"""
    print("🚀 بدء نظام نشر الأخبار التلقائي - النسخة 17")
    print("🇮🇶 أخبار عراقية فقط مع إطارات مخصصة")
    print("=" * 50)
