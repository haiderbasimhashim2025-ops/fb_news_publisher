...
# -*- coding: utf-8 -*-
"""
نظام نشر الأخبار التلقائي على Facebook - النسخة 16
...
# مصادر الأخبار العراقية (مركزة على العراق)
NEWS_SOURCES = [
    # المصادر الإخبارية العراقية الرسمية
    'https://feeds.aljazeera.net/xml/news/iraq.xml',  # الجزيرة - أخبار العراق
    'https://feeds.bbc.co.uk/news/world/middle_east/rss.xml',  # BBC - الشرق الأوسط (يشمل العراق)
    'https://feeds.reuters.com/reuters/worldNews',  # رويترز - أخبار عالمية (يشمل العراق)
    'https://feeds.aljazeera.net/xml/news/middleeast.xml',  # الجزيرة - الشرق الأوسط
    
    # المصادر الإخبارية العراقية المحلية
    'https://www.iraqinews.com/feed/',  # Iraqi News
    'https://www.rudaw.net/arabic/rss.xml',  # Rudaw عربي
    'https://www.alsumaria.tv/rss.xml',  # قناة السومرية
    'https://www.aliraqiya.iq/rss.xml',  # الإذاعة والتلفاز العراقية
    
    # وكالات الأنباء العراقية
    'https://www.ina.iq/rss.xml',  # وكالة الأنباء العراقية
    'https://www.spa.gov.iq/rss.xml',  # وكالة الصحافة العراقية
    
    # المصادر الإخبارية الإقليمية التي تغطي العراق
    'https://feeds.aljazeera.net/xml/news/arabworld.xml',  # الجزيرة - العالم العربي
    'https://www.dw.com/ar/s-100625/rss.xml',  # DW عربي
    'https://www.bbc.com/arabic/index.xml',  # BBC عربي
    'https://feeds.reuters.com/reuters/arabicNews',  # رويترز عربي
    
    # المصادر المتخصصة
    'https://feeds.aljazeera.net/xml/news/business.xml',  # الجزيرة - الاقتصاد
    'https://feeds.aljazeera.net/xml/news/sports.xml',  # الجزيرة - الرياضة
    'https://feeds.aljazeera.net/xml/news/technology.xml',  # الجزيرة - التكنولوجيا
    'https://feeds.bbc.co.uk/news/health/rss.xml',  # BBC - الصحة
    'https://feeds.aljazeera.net/xml/news/entertainment.xml',  # الجزيرة - الترفيه
    'https://feeds.reuters.com/reuters/entertainment',  # رويترز - الترفيه
]

# كلمات مفتاحية عراقية للتركيز عليها
IRAQ_KEYWORDS = [
    'العراق', 'بغداد', 'البصرة', 'الموصل', 'أربيل', 'كركوك', 'النجف', 'كربلاء',
    'الحلة', 'الناصرية', 'الديوانية', 'الرمادي', 'تكريت', 'سامراء', 'هيت',
    'عنة', 'الفلوجة', 'الرطبة', 'القائم', 'الحديثة', 'الخالدية', 'الرمادي',
    'العراقي', 'العراقيين', 'عراقي', 'حكومة العراق', 'البرلمان العراقي',
    'الجيش العراقي', 'الشرطة العراقية', 'الحشد الشعبي', 'البيشمركة',
    'الدينار العراقي', 'النفط العراقي', 'الاقتصاد العراقي', 'السياحة العراقية',
]
...
def fetch_news():
    ...
    for source_url in NEWS_SOURCES:
        try:
            ...
            for entry in feed.entries[:10]:  # أول 10 أخبار من كل مصدر
...
def main_loop():
    """الحلقة الرئيسية"""
    print("🚀 بدء نظام نشر الأخبار التلقائي - النسخة 16")
    print("🇮🇶 تركيز على الأخبار العراقية فقط")
    print("=" * 50)
    ...
    while True:
        try:
            # جلب الأخبار الجديدة
            print(f"\n⏰ {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - جاري جلب الأخبار العراقية...")
            news_list = fetch_news()
            print(f"✅ تم جلب {len(news_list)} خبر عراقي")
            ...
            if saved_count > 0:
                print(f"💾 تم حفظ {saved_count} أخبار عراقية جديدة")
            ...
            if unposted:
                ...
                print("⏳ لا توجد أخبار عراقية جديدة للنشر الآن")
