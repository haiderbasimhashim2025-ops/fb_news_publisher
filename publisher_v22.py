...
# -*- coding: utf-8 -*-
"""
نظام نشر الأخبار السريع جداً - النسخة 22
🚀 تحسينات السرعة:
                ...
                if is_arabic(title) or is_arabic(description):
                    if is_iraq_related(title) or is_iraq_related(description):
                        if not is_duplicate(title, description, link):
                            ...
                            elif hasattr(entry, 'media_thumbnail') and entry.get('media_thumbnail'):
                                image_url = entry.media_thumbnail[0].get('url')
                            elif 'image' in entry:
                                image_url = entry.image.get('href') or entry.image.get('url')
                            elif hasattr(entry, 'enclosures') and entry.get('enclosures'):
                                for enc in entry.enclosures:
                                    if enc.get('type', '').startswith('image/'):
                                        image_url = enc.get('url') or enc.get('href')
                                        break
                            # استخراج الصورة من محتوى HTML
                            if not image_url and entry.get('summary'):
                                soup_img = BeautifulSoup(entry.get('summary', ''), 'html.parser')
                                img_tag = soup_img.find('img')
                                if img_tag:
                                    image_url = img_tag.get('src') or img_tag.get('data-src')
                            if not image_url and entry.get('content'):
                                for c in entry.get('content', []):
                                    soup_img = BeautifulSoup(c.get('value', ''), 'html.parser')
                                    img_tag = soup_img.find('img')
                                    if img_tag:
                                        image_url = img_tag.get('src') or img_tag.get('data-src')
                                        break
...
def download_image_fast(image_url):
    ...
    try:
        ...
        response = requests.get(image_url, timeout=6, headers=headers)
        ...
        if news_image:
            ...
            for xi in range(0, iw, 80):
                for yi in range(0, ih, 80):
                    bg_draw.ellipse([xi-2, yi-2, xi+2, yi+2], fill=(*dc, ) if len(dc)==3 else dc, outline=None)
            # خطوط قطرية خفيفة
...
# =====================================================
def main_loop_fast():
    print("🚀 بدء نظام النشر السريع جداً - النسخة 22")
