# -*- coding: utf-8 -*-
"""
نظام نشر الأخبار التلقائي على فيسبوك - النسخة 9
✅ يعمل كل 15 دقيقة بدلاً من 5 دقائق
✅ تنظيف تلقائي مستمر لقاعدة البيانات
...
def cleanup_old_records():
    """حذف الأخبار القديمة (أكثر من 24 ساعة) - تنظيف مستمر"""
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        
        # حذف السجلات الأقدم من 24 ساعة (بدلاً من 48)
        ...
        print(f"📊 إجمالي السجلات في قاعدة البيانات: {total_records}")
...
def get_news():
    ...
    for source in NEWS_SOURCES:
        try:
            ...
            for entry in feed.entries[:3]:  # أول 3 أخبار من كل مصدر (زيادة من 2)
...
def post_to_facebook(page_id, page_config, title, image_url=None):
    ...
    try:
        ...
        if post_image:
            ...
            if response.status_code == 200:
                print(f"✅ {page_config['name']}: {title[:50]}")
                return True
            else:
                print(f"❌ {page_config['name']}: {response.text[:100]}")
                return False
        else:
...
def main():
    """البرنامج الرئيسي"""
    print(f"\n[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 🚀 بدء دورة النشر (كل 15 دقيقة)...")
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 📡 عدد المصادر: {len(NEWS_SOURCES)}")
    
    # إنشاء قاعدة البيانات
    init_db()
    
    # تنظيف السجلات القديمة (تنظيف مستمر)
    cleanup_old_records()
        ...
        for news_item in news:
            for page_id, page_config in PAGES_CONFIG.items():
                if post_to_facebook(page_id, page_config, news_item["title"], news_item.get("image_url")):
                    published_count += 1
                time.sleep(2)  # تأخير 2 ثانية بين المنشورات
        print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] ✅ تم نشر {published_count} منشور")
    else:
        print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] ℹ️ لا توجد أخبار جديدة في هذه الدورة")
    
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] ✅ دورة النشر انتهت بنجاح\n")
